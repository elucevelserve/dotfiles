...
Singleton {
  ...
  // Whether any screen backlight device exists (false on VMs/desktops without
  // one). Checked once at startup via brightnessctl -l.
  ...
  // One-time check for backlight devices
  Process {
    id: detectProcess
    command: ["brightnessctl", "-l"]

    onExited: {
      root.hasBacklight = detectStdout.text.indexOf("class 'backlight'") !== -1
    }
