...
Singleton {
  ...
  function refresh() {
    if (!detectThemeProcess.running) {
      detectThemeProcess.running = true
    }
  }

  Component.onCompleted: refresh()

  // Event-driven: react to portal SettingChanged instead of polling.
  // dbus-monitor is part of systemd, always present on Arch.
  Process {
    running: true
    command: [
    ...
    stdout: SplitParser {
      onRead: root.refresh()   // signal received → re-read the value
    }
  }
