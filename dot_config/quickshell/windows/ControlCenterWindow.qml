...
PopupWindow {
  ...
  anchor.window: ControlCenterState.anchorWindow
  anchor.rect.x: ControlCenterState.anchorX
  anchor.rect.y: ControlCenterState.anchorY

  // One-way sync: outside-click dismissal (popup_done) hides us, so reflect
  // that in state. The bar widgets drive opening via ControlCenterState.openAt().
  onVisibleChanged: {
    if (!visible) {
  ...
  Shortcut {
    sequence: "Escape"
    onActivated: ControlCenterState.visible = false
  }
